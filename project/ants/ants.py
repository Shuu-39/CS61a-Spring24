...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if original_ant.is_container and original_ant.can_contain(self):#本地蚂蚁可以做容器
                original_ant.store_ant(self)
            elif self.is_container and self.can_contain(original_ant):      #新放入蚂蚁可做容器
                place.ant = self
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not (current_place is None or current_place.is_hive): 
            if self.lower_bound <= self.distance <= self.upper_bound and len(current_place.bees) > 0:
...
class FireAnt(Ant):
    ...
    def reduce_health(self, amount = 1):
        ...
        # 2. 调用父类的reduce_health，触发生命值检查和zero_health_callback
        super().reduce_health(amount)

    def zero_health_callback(self):
        """FireAnt死亡时触发:先调用父类回调，再执行爆炸逻辑"""
        # 关键：先调用Insect的zero_health_callback，确保FireAnt被正确移除，避免死后仍对蜜蜂造成伤害
        super().zero_health_callback()
        
        # 3. 爆炸伤害：对当前位置存活蜜蜂造成3点伤害
        if self.place is not None:
            bees = list(self.place.bees)
            for bee in bees:
                if bee.health > 0:
...
class TankAnt(BodyguardAnt):
    ...
    def action(self, gamestate):
        # 1. 首先执行BodyguardAnt的保护行为（如果有被保护的蚂蚁，则先让它行动）
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        # BEGIN Problem Optional 2
        dict_of_insects_in_front = {}
        current_place = self.place
        distance = 0
        while (not current_place.is_hive) and (current_place is not None):
            ant = current_place.ant
            bees = current_place.bees

            if ant is not None and ant is not self:
                dict_of_insects_in_front[ant] = distance
                
            if bees is not None:
                for bee in bees:
                    if bee.health > 0:
                        dict_of_insects_in_front[bee] = distance
            current_place = current_place.entrance
            distance += 1
        return dict_of_insects_in_front
        # END Problem Optional 2
