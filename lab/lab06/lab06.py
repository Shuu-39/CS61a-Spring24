class Transaction:
    ...
    def report(self):
        ...
        if self.changed():
            "*** YOUR CODE HERE ***"
            msg = f"{'increased ' if self.after > self.before else 'decreased '}{self.before}->{self.after}"
...
class ChangeMachine:
    ...
    def change(self, coin):
        ...
        "*** YOUR CODE HERE ***"
        # 第一步：将投币放入零钱池（你的原逻辑，保留）
        if coin in self.coins:
            self.coins[coin] += 1
        else:
            self.coins[coin] = 1
        
        # 第二步：调用make_change找零，得到找零列表
        cost = make_change(coin, self.coins)
        # 处理极端情况（题目说机器总能找零，可省略，但加了更健壮）
        if not cost:
            cost = [coin]
        
        # 第三步：手动统计找零使用的硬币数量（替代Counter）
        used = {}
        for c in cost:
            used[c] = used.get(c, 0) + 1  # 关键：get方法，无键则默认0
        
        # 第四步：优雅更新self.coins，实现字典相减，删除0计数键
        for c in used:
            self.coins[c] -= used[c]  # 原地减法，不丢失其他键
            if self.coins[c] == 0:    # 计数为0，删除该键，保持规范
                del self.coins[c]
